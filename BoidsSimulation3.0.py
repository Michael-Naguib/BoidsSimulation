...
import pygame                               # Get keyboad input
...
if __name__ == "__main__":
    ...
    TIME_STEP = 0.15    # The change in time to consider when using Euler's Method for approximating the kinematics
    MAX_TIME_SEC = 25   # Specify the ammount of time to run the simulation for ...
    ...
    HEIGHT = 600
    WIDTH = round(HEIGHT)# * 1.6180339) # Pick nice aspect ratio by using phi= THE GOLDEN RATIO (one of my favorite numbers)
    DEPTH = WIDTH
    DIMENSIONS = [HEIGHT,WIDTH,DEPTH]

    BOID_QUANTITY= 70        # The number of boids in the simulation
    ...
    MAX_ACEL = 2*9.81         # Specify the maximum (magnitude) acceleration that can be achieved in the acceleration
    MAX_VEL = 2*MAX_ACEL    # Limit the velocity (magnitude)
    ...
    REL_RANGES = [5/6,1/6,3/6]              # Specify the Relative distances of each rule
    REL_RULE_WEIGHTS = [1/6,3/6,2/6]        # Specify the Relative weights of each Rule
    ...
    if VISUALIZE and not PRECOMPUTE:
        vis = VisualizeSqarm.VisualizeSwarm(frameDelay=FRAME_DELAY,box=DIMENSIONS,pointSize=BOID_SCALE)
    ...
    while time.time()-startTime < MAX_TIME_SEC:
        ...
        for indx in range(len(boids)):
            ...
            boids[indx].pos = pos
            # TODO: position check to handle exploding simulations...
